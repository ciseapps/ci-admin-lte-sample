...
class Content extends AbstractPart
{
    ...
    public function write()
    {
        ...
        $xmlWriter->startElement('text:app_table_sequence-decls');
        foreach ($sequences as $sequence) {
            $xmlWriter->startElement('text:app_table_sequence-decl');
            $xmlWriter->writeAttribute('text:display-outline-level', 0);
            $xmlWriter->writeAttribute('text:name', $sequence);
            $xmlWriter->endElement();
        }
        $xmlWriter->endElement(); // text:app_table_sequence-decl
